...
neuron_list = neuron_list(randperm(length(neuron_list)));
 % Define an anonymous function to update the cells
 update_connections = @(c, new_val) [c, new_val];
...
%     number_of_divergent_neuron_mean=data_L.(fn_L{neuronconnectiontypiteration}).number_of_divergent_neuron_mean;%  Divergence refers to a single neuron forming synaptic connections with multiple target neurons.
%     number_of_convergent_neuron_mean=data_L.(fn_L{neuronconnectiontypiteration}).number_of_convergent_neuron_mean;% Convergence refers to multiple neurons forming synaptic connections with a single target neuron.
...
connection_array(pre_idx) = cellfun(@(c, new_val) update_connections(c, new_val), ...
                                     connection_array(pre_idx), num2cell(post_idx), 'UniformOutput', false);
...
cubic_array = cell(cube_size, cube_size, cube_size);
index = 1;
for x = 1:cube_size
    for y = 1:cube_size
        for z = 1:cube_size
            if index <= total_neurons
                cubic_array{x, y, z} = neuron_list{index};
                index = index + 1;
            else
                cubic_array{x, y, z} = [];
            end
