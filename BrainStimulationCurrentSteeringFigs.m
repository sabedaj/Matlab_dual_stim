...
%% for rasterplots
%flash
%E:\DATA\Rat_019\flash_001_210511_100735
Flash_raster(9) %only plotted the first 200 trials, this will plot all 900

%stim
%E:\DATA\Rat_019\S1E4_9elect_h_001_210511_161244
generate_StackedRaster_sab(9,6) %plotted chn 9, 6uA
...
%function. To change to baseline activity, alter Csplit line 216 to basepk_all
...
figure
stdshade((allresp'./alltog').*100,0.2,'k')
ylim([0 100])
xlim([1 16])
text(2,45,['N=' num2str(nansum(alltog))])
...
figure%distance based sig elect
stdshade([distsigelect_array(1:19,:).*100./numdistsigelect_array(1:19,:)]',0.2,'k',distancefrelect)
ylim([0 100])
ylabel('Precentage responding electrodes (%)')
...
for sepdist=5:2:9
    ...
    for current=2:length(AMP)
        currcheck=['C' num2str(AMP(current))];
        currsepavgshift.(sepcheck) (current)=abs(mean(diff(centroidpos_all.(sepcheck).(currcheck)),'all'));
        currseperrorshift.(sepcheck) (current)=std(diff(centroidpos_all.(sepcheck).(currcheck)),[],'all')./sqrt(numel(diff(centroidpos_all.(sepcheck).(currcheck))));
    end
    figure(5)
    hold on
...
%centroidpod_all needs to be altered to be only the stimulating electrodes
%and thos between the stimulating electrodes
trialstoplot=1:5;
figure
hold on
stdshade(flipud(centroidpos_all.sep5.C6(trialstoplot,:))'./(300).*100,0.2,'r',trialstoplot);
stdshade(flipud(centroidpos_all.sep7.C6(trialstoplot,:))'./(400).*100,0.2,'b',trialstoplot);
stdshade(flipud(centroidpos_all.sep9.C6(trialstoplot,:))'./(500).*100,0.2,'k',trialstoplot);

x=trialstoplot;
y=mean([flipud(centroidpos_all.sep5.C6(trialstoplot,:))./300.*100 flipud(centroidpos_all.sep7.C6(trialstoplot,:))./400.*100 mean(flipud(centroidpos_all.sep9.C6(trialstoplot,:))./500.*100,2)],2);
p = polyfit(x,y,1); 
yfit = polyval(p,x); 
plot(x,yfit,'g')
numparam=2;
...
labels=labels(trials);
xticklabels(labels)
ylabel('% stimulating electrode separation distance')
...
figure
hold on
stdshade(flipud(centroidpos_all.sep5.C6)',0.2,'r',1:5);
stdshade(flipud(centroidpos_all.sep7.C6)',0.2,'b',1:5);
stdshade(flipud(centroidpos_all.sep9.C6)',0.2,'k',1:5);
xticks(1:5)
labels=fliplr({'0:100','25:75','50:50','75:25','100:0'});
labels=labels(trials);
xticklabels(labels)
...
for sepdist=5:2:9
    ...
    for current=1:length(AMP)
        ...
        for trial=trials
            check=['T' num2str(trial)];
            dat=saveshanksepdist.(sepcheck).(currcheck).(check).D1;
            dat(isinf(dat))=nan;
%            clear pairavgcur erpairavgcur
%             for paircount=1:size(dat,2)/4
%                 pairavgcur(:,paircount)=nanmean(dat(:,( paircount-1)*4+1:4* paircount),2);
%             end
            ...
            pairavg_dwnsample=dat(:, samples_rand.(sepcheck));% downsample%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            pairavg_dwnsample(sum(~isnan(pairavg_dwnsample),2)<10,:)=nan; %remove rows with less than 10 pairs in the average
            pairavgdwnsample_all_nomean.(sepcheck).(currcheck).(check)=pairavg_dwnsample;
            pairavgdwnsample_all.(sepcheck).(currcheck)(:,trial)=nanmean(pairavg_dwnsample,2);
    ...
    for current=1:length(AMP)
        ...
        avgpcurr=mean(centroidpos_all.(sepcheck).(currcheck),2);
        stdavg=std(centroidpos_all.(sepcheck).(currcheck),[],2)./sqrt(size(centroidpos_all.(sepcheck).(currcheck),2));
        peak_all.(sepcheck).(currcheck)=nan(size(saveshanksepdist.(sepcheck).(currcheck).(check).D1));
           %%%%%%%%%%%%%%%%%smoothed peaks
           for trial=trials
               check=['T' num2str(trial)];
               dat=saveshanksepdist.(sepcheck).(currcheck).(check).D1;
...
%     
            ...
            stdavg=nanstd(peak_all.(sepcheck).(currcheck),[],2)./sqrt(size(peak_all.(sepcheck).(currcheck),2));
...
labels=labels(trials);
xticklabels(labels)
ylabel('Centroid model error (\mum)')
