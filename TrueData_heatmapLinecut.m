function TrueData_heatmapLinecut(AMPInterestSingleLinePlot,avgnospT,stderrspktrial,startpointseconds, secondstoanalyse)
...
singleLineplotvalstd=zeros(nChn,trialjump);
 for group_related=1:endtrialelect*2:maxid*2 %group_related is used to go through groups of related trials
     for TJ_related=1:2:trialjump*2 %goes through trials related by trial jump
         ...
         DepthChangeingSpiking_SM(normalisedAvgspikingT, chosen_trials,fignum,max(cell2mat(TrialParams(:,2))),chosen_trials_amp); %plots heat maps
