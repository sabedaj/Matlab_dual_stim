...
for folder=1:length(sigmoidAll)
    ...
    for recchn=chnnrange(1):chnnrange(end)
        ...
        for stimchn=1:length(fns)
            count=count+1;
            if length(sigmoidAll{folder}.(chnname).(fns{stimchn}))==5
                sigmoidplot(count,1:6)=[nan sigmoidAll{folder}.(chnname).(fns{stimchn})];
            else
                sigmoidplot(count,1:6)=sigmoidAll{folder}.(chnname).(fns{stimchn});
            end
        end
    end
end
figure
plot([0; amp],nanmean(sigmoidplot))
xlabel('current (uA)')
...
stddistcurrent=nan(20,6);
 ...
 figure(folder);hold on; errorbar(0:50:950,splitdist{current},stdsplitdist{current})
