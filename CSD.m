function [CSDoutput]  = CSD(data,SR,spacing,varargin)
    ...
    scV = plot(repmat(max(xAxis),2,1),linspace(0,scale1,2)','b'); 
    scV.LineWidth = 3; 
    % figure labels 
    ylabel('Electrode');
    title({'Voltage'; ['(\color{blue}scale bar = ' num2str(scale1)...
        ' V\color{black})']});
    xlabel('Time (ms)');
    ...
    % plots scale bar at bottom right of plot
    scC = plot(repmat(max(xAxis),2,1),[0 scale2]','b'); 
    scC.LineWidth = 3;  
    % figure labels
    ylabel('Electrode')
    title({'CSD'; ['(\color{blue}scale bar = ' num2str(scale2) ...
