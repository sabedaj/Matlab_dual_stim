...
for tID=starttrial:trialjump:endtrial
    ...
    if isempty(TrialParams(cell2mat(TrialParams(1:end,2)) == tID))
        ...
        if any(TrialParamstID>length(trig))
            warning('Triggers are missing')
            TrialParamstID(TrialParamstID>length(trig))=[];
        end
        ...
        for indT=1:nTrig
            for chsp=chnrange
                ...
                rate=v(((v(:,1)>(trigtID(indT)-90))&(v(:,1)<(trigtID(indT)+300))),1)-trigtID(indT);
...
%                 end
                ...
                if ~isempty(rate)
                rateall(chsp,1:391,indT)=hist(rate,-90:300);
                else
                    rateall(chsp,1:391,indT)=zeros(391,1);%rateall(chsp,1:181,indT)=zeros(181,1);
                end
                spike=0;
            end
        end
